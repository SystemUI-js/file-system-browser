...
function parseWriteOpt(options?: WriteOpt): { encoding?: BufferEncoding | null } {
  if (!options) return {};
  if (typeof options === 'string') return { encoding: options };
  return { encoding: options.encoding };
}
...
function listDirectChildren(entries: Map<string, FileEntry>, path: string): FileEntry[] {
  path = norm(path);
  return Array.from(entries.values()).filter((entry) => entry.parentPath === path);
}

export const createMemoryStoragePlugin: FsPluginFactory<MemoryStoragePluginOptions> = (
  options,
  ctx: FsPluginContext
) => {
  ...
  function ensureParentDirectory(path: string, operation: string): string {
    ...
    if (!parentEntry) {
      throw new Error(`ENOENT: no such file or directory, ${operation} '${parent}'`);
    }
  ...
  function hardLinkSiblings(entry: FileEntry): FileEntry[] {
    ...
    return Array.from(entries.values()).filter(
      (candidate) => candidate.type === 'file' && candidate.hardLinkKey === entry.hardLinkKey
    );
  }
  ...
  function readFileInternal(path: string): Uint8Array {
    ...
    return new Uint8Array(entry.content ? cloneBytes(entry.content) : new ArrayBuffer(0));
  }
  ...
  function mkdirInternal(path: string, recursive?: boolean): void {
    ...
    if (existing) {
      if (existing.type !== 'directory') throw new Error(`EEXIST: file already exists, mkdir '${path}'`);
      return;
    }
    const parent = parentOf(path);
    const parentEntry = entries.get(parent);
    if (!parentEntry) {
      ...
      else throw new Error(`ENOENT: no such file or directory, mkdir '${parent}'`);
  ...
  function deleteRecursive(path: string, recursive?: boolean, force?: boolean): void {
    path = norm(path);
  ...
  function renameInternal(oldPath: string, newPath: string): void {
    ...
    if (existing?.type === 'directory' && listDirectChildren(entries, newPath).length) {
    ...
    if (entry.type === 'directory') {
      ...
      for (const child of descendants) {
        ...
        const childNewPath = norm(`${newPath}${child.path.slice(oldPath.length)}`);
        const updated: FileEntry = {
          ...cloneEntry(child),
          path: childNewPath,
  ...
  function readFd(fd: number, buffer: Uint8Array, offset: number, length: number, position: number | null) {
  ...
  function writeFd(
    ...
    const buf = typeof bufOrStr === 'string' ? toBuffer(bufOrStr) : toBuffer(bufOrStr);
    const toWrite =
      length != null && offset != null ? buf.subarray(offset, offset + length) : buf;
    const current = entries.has(state.path) ? readFileInternal(state.path) : new Uint8Array();
    const start = position ?? state.position;
    const needed = start + toWrite.length;
    const next = current.length < needed ? new Uint8Array(needed) : current.slice();
  ...
  function readdirHandler(
    path: string,
    options?: { withFileTypes?: false; encoding?: BufferEncoding } | BufferEncoding
  ): Promise<string[]>;
  async function readdirHandler(
    path: string,
    options?: { withFileTypes?: boolean; encoding?: BufferEncoding } | BufferEncoding
  ...
  return {
    ...
    handlers: {
      ...
      async writeFile(file: string | number, data: Iterable<number>, options?: WriteOpt) {
      ...
      async appendFile(file: string | number, data: Iterable<number>, options?: WriteOpt) {
        const { encoding } = parseWriteOpt(options);
        const path = typeof file === 'number' ? fdStates.get(file)?.path : file;
        if (!path) throw new Error('EBADF: bad file descriptor, write');
        const base = entries.has(norm(path)) ? readFileInternal(path) : new Uint8Array();
        writeFileInternal(path, BufferPolyfill.concat([base, toBuffer(data, encoding)]));
      },
      ...
      async mkdir(path: string, options?: number | string | { recursive?: boolean }) {
        mkdirInternal(path, typeof options === 'object' ? !!options.recursive : false);
      },
      ...
      async rmdir(path: string, options?: { recursive?: boolean }) {
        const entry = entries.get(norm(path));
        if (entry && entry.type !== 'directory') throw new Error(`ENOTDIR: not a directory, rmdir '${path}'`);
      ...
      async symlink(target: string, path: string) {
        ...
        if (entries.has(path)) throw new Error(`EEXIST: file already exists, symlink '${path}'`);
      ...
      async link(existingPath: string, newPath: string) {
        ...
        if (!entry) throw new Error(`ENOENT: no such file or directory, link '${existingPath}'`);
        if (entry.type !== 'file') throw new Error(`EPERM: hard link target must be a file, got '${existingPath}'`);
        ensureParentDirectory(newPath, 'link parent');
        if (entries.has(newPath)) throw new Error(`EEXIST: file already exists, link '${newPath}'`);
        const key = entry.hardLinkKey || entry.path;
        if (!entry.hardLinkKey) entries.set(entry.path, { ...entry, hardLinkKey: key });
        const linked = fileEntry(newPath, new Uint8Array(entry.content ?? new ArrayBuffer(0)), null, key);
      ...
      async access(path: string) {
        if (!resolveSymlink(path).entry) throw new Error(`ENOENT: no such file or directory, access '${path}'`);
      },
      ...
      async open(path: string, flags: string) {
        ...
        if (!existing && /[wa]/.test(flags)) writeFileInternal(path, new Uint8Array());
      ...
      async read(fd: number, buffer: Uint8Array, offset: number, length: number, position: number | null) {
        return readFd(fd, buffer, offset, length, position);
      },
      async write(fd: number, buffer: Uint8Array | string, offset?: number, length?: number, position?: number | null) {
        return writeFd(fd, buffer, offset, length, position);
      },
      async close(fd: number) {
        if (!fdStates.has(fd)) throw new Error('EBADF: bad file descriptor, close');
        fdStates.delete(fd);
        ctx.releaseFd(fd);
      },
      ...
      createReadStream(path: string, opts?: { highWaterMark?: number }) {
        ...
        return {
          on<E extends keyof ReadStreamEvents>(event: E, handler: ReadStreamEvents[E]) {
          ...
          pipe(dest: { write: (chunk: Uint8Array | BufferPolyfill | string) => unknown; end?: () => unknown }) {
      ...
      createWriteStream(path: string) {
        ...
        return {
          ...
          on<E extends keyof WriteStreamEvents>(event: E, handler: WriteStreamEvents[E]) {
            listeners[event].push(handler);
            return this;
          },
        };

...
class SortService {
  ...
  async setConfig(
    dir: string,
    patch: Partial<Omit<DirSortConfig, 'dir' | 'updatedAt'>>
  ): Promise<DirSortConfig> {
    const d = normDir(dir);
  ...
  async updateIconPosition(
    dir: string,
    key: string,
    pos: IconPosition | null
  ): Promise<DirSortConfig> {
    const cfg = await this.getConfig(dir);
    const next = { ...(cfg.iconPositions || {}) } as Record<
      string,
      IconPosition
    >;
    if (pos) next[key] = pos;
  ...
  async onEntriesMoved(
    ...
    const srcIcons = { ...(srcCfg.iconPositions || {}) } as Record<
      string,
      IconPosition
    >;
    for (const k of keys) delete srcIcons[k];
    await this.setConfig(s, {
      manualOrder: srcManual,
      iconPositions: srcIcons,
    });
    ...
    const destIcons = { ...(destCfg.iconPositions || {}) } as Record<
      string,
      IconPosition
    >;
    for (const k of keys) delete destIcons[k];
    await this.setConfig(d, {
      manualOrder: destManual,
      iconPositions: destIcons,
    });
  }
  ...
  async onEntriesRemoved(dir: string, keys: string[]): Promise<void> {
    ...
    const icons = { ...(cfg.iconPositions || {}) } as Record<
      string,
      IconPosition
    >;
    ...
    if (mode === 'manual') {
      ...
      return arr.sort(
        (a, b) =>
          ...
          (rank.get(this.keyOf(b)) ?? Number.MAX_SAFE_INTEGER)
      );
    }
